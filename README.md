...
When we extracted the deployed assets and inspected the source maps, they turned out to be a React + Vite + TypeScript SPA. Because the structure matched a Power Apps Code App project closely, the source files could be dropped directly into a **Power Apps Code App** Vite template with minimal changes. You get the best of both worlds: the polished UI generated by the Frontier builder, plus the full Power Apps SDK, Dataverse connectors, and Local Play integration provided by the Code App toolchain.

> **Note:** The deployed files are transformed (minified, hashed filenames) but often ship with **source maps** that let you recover the original TypeScript source.
