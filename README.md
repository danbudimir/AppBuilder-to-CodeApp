# Rebuilding a Power Apps Code App from a Deployed URL

This guide documents how to take a live, deployed Power Apps Code App — one you only have a browser URL for — and turn it into a fully working local Vite + React + TypeScript project that is integrated with the Power Apps SDK, wired to Dataverse, and runnable inside the Power Apps host via Local Play.
...
Power Apps Code Apps are React + Vite single-page apps hosted on the Power Apps gateway. The deployed files are transformed (minified, hashed filenames) but often ship with **source maps** that let you recover the original TypeScript source.

The starting URL in this walkthrough looked like:
...
## Step 2 — Scaffold a Local Vite Project

Create a fresh directory and set up the project skeleton:
...
Then install the Power Apps SDK packages:
...
Paste or place the recovered `.tsx` / `.ts` / `.css` files into `src/`. The typical structure you'll find in a Code App:
